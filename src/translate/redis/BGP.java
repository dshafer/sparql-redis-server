...
public class BGP implements RedisOP {
	...
	public BGP() {
		...
		bgpMatchScript = ""
				...
				+ "    joinSig = '' \n"
				+ "    -- redis.call('rpush', logKey, 'constructing join hash:' .. l) \n"
				+ "    for i, joinCol in ipairs(joinCols) do \n"
				+ "      -- redis.call('rpush', logKey, 'lval[joinCol[2]] = ' .. lval[joinCol[2]]) \n"
				...
				+ "      end \n"
				+ "      for ri,rKeepItem in ipairs(rightKeepCols) do \n"
				+ "        table.insert(outputRow, rKeepItem) \n"
				+ "      end \n"
				...
				+ "    redis.call('rpush', logKey, 'splitting pattern: ' .. pattern) \n"
				+ "    table.insert(patternResult, split(pattern, ':')) \n"
				+ "  end \n"
				+ "  table.insert(tripleTables, patternResult) \n"
				+ "end \n"
				+ "for i, patternResult in ipairs(tripleTables) do \n "
				+ "  redis.call('rpush', logKey, 'triple pattern[' .. i .. '] = ' .. cjson.encode(patternResult)) \n"
				+ "end \n"
				+ " \n"
				...
				+ "redis.call('set', graphPatternKey, cjson.encode(result)) \n"
				+ "-- we're done \n"
				+ "return 1 \n"
				+ "";
